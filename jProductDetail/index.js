...
var jProductDetailInterval;
function jProductDetailIntervalEvent(){
  if(jProductDetailInterval) {
    clearInterval(jProductDetailInterval);
    jProductDetailInterval = setInterval(function(){
      document.getElementById("jProductDetailCont").scrollTop=document.getElementById("jProductDetailCont").scrollTop+3;
    }, 60);
  }else{
    jProductDetailInterval = setInterval(function(){
      document.getElementById("jProductDetailCont").scrollTop=document.getElementById("jProductDetailCont").scrollTop+3;
  ...
  render: function (data, config) {
    ...
    if(data.contentType != 3){
      ...
      if(data.photoUrls && data.content){ //图片和文字
        ...
        for(var i =0;i<textC.length;i++){
          html+=`<p style="text-indent: 2.2em;font-size:30px;line-height:40px;letter-spacing:4px;margin-bottom:10px;">${textC[i]}</p>`
          if(img[i]){
            html+= `<img style="width:100%;height:300px;" src="${img[i]}" />`
          }
        }
